  ...
  return (
    <>
      ...
      <section className="profile-bottom" >
        ...
        <button className="reset-password-button" name="signin"  >Reset Password</button>
      </section>
    </>
  )
}
...
function ProfileEdit(props) {
  ...
  async function process_change() {
    const { email, avatar, status } = formState;
    ...
    if (!email) {
      ...
      // }
      if (formState.avatar) {
        let result = await Auth.currentAuthenticatedUser().then();
    ...
    if (email) {
      ...
      context.updateUser(await Auth.currentAuthenticatedUser())
