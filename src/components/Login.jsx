  ...
  return (
    <>
      ...
      <section className="form-bottom" onKeyPress={process_keypress} >
        ...
        <span name="recover_password" onClick={() => props.updateFormState("recover")}>Forgot your password?</span>
...
function FormPassRecover(props) {
  ...
  async function process_confirm() {
    ...
    try {
      await Auth.forgotPasswordSubmit(username, authCode, new_password).then(() => props.updateFormState("login"));
    } catch (e) {
      setFormState(() => ({ ...formState, error_message: e.message }));
    }
  }
